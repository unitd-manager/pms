...
/**
 * Leave & Permission Report page.
 *
 * Drop this file into your PMS React project (e.g. src/pages/LeavePermissionReport.jsx)
...
export default function LeavePermissionReport() {
  const now = new Date();
  ...
  async function fetchReport(y, m) {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/reports/getLeavePermissionReport?year=${y}&month=${m}`);
  ...
  function handleExport() {
    if (!data) return;
    const rows = [
      ['#', 'Staff Name', 'Month', 'Leave', 'Permission'],
      ...data.staffList.map((s, i) => [i + 1, s.name, data.monthLabel, s.leave, s.permission]),
      ['', 'Total', data.monthLabel, data.totalLeaves, data.totalPermissions],
    ];
    ...
    a.href = url;
    a.download = `leave-permission-${data.monthLabel.replace(' ', '-')}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }

  const maxLeaveForBar = data
    ? Math.max(...data.leastLeaveTop5.map((s) => s.leave), 1)
    : 1;
    ...
    <div style={styles.page}>
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div>
            <h1 style={styles.title}>Leave &amp; Permission Report</h1>
            <p style={styles.subtitle}>Summary of leave and permission details by staff for the selected month and year</p>
          </div>
        </div>
      ...
      {!loading && !error && data && (
        <>
          <div style={styles.cardsRow}>
            ...
            <StatCard icon="📋" iconBg="#fef3c7" label="Total Permissions" value={data.totalPermissions} sub="This Month" />
            <StatCard icon="📅" iconBg="#d1fae5" label="Total Leaves" value={data.totalLeaves} sub="This Month" />
          </div>

          <div style={styles.mainRow}>
            <div style={styles.tableCard}>
              <h3 style={styles.cardTitle}>{data.monthLabel.toUpperCase()} - LEAVE &amp; PERMISSION DETAILS</h3>
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <th style={{ ...styles.th, textAlign: 'right' }}>Leave</th>
                    <th style={{ ...styles.th, textAlign: 'right' }}>Permission</th>
                  </tr>
                </thead>
                <tbody>
                  {data.staffList.map((s, i) => (
                    <tr key={i}>
                      ...
                      <td style={styles.td}>{data.monthLabel}</td>
                      <td style={{ ...styles.td, textAlign: 'right' }}>{s.leave}</td>
                      <td style={{ ...styles.td, textAlign: 'right' }}>{s.permission}</td>
                    </tr>
                  ))}
                  <tr style={{ background: '#f9fafb', fontWeight: 700 }}>
                    ...
                    <td style={styles.td}>{data.monthLabel}</td>
                    <td style={{ ...styles.td, textAlign: 'right' }}>{data.totalLeaves}</td>
                    <td style={{ ...styles.td, textAlign: 'right' }}>{data.totalPermissions}</td>
                  </tr>
                </tbody>
              </table>
              <p style={styles.footerNote}>
                Showing data for {data.monthLabel} &nbsp;|&nbsp; Total Staff: {data.totalStaff} &nbsp;|&nbsp; Total Permissions: {data.totalPermissions} &nbsp;|&nbsp; Total Leaves: {data.totalLeaves}
              </p>
            </div>

            <div style={styles.sideCol}>
              <div style={styles.sideCard}>
                <h3 style={styles.cardTitle}>STAFF LEAVE &amp; PERMISSION SUMMARY (TOP 5)</h3>
                <p style={styles.sideSub}>Top 5 staff with the least leave taken</p>
                <BarList
                  items={data.leastLeaveTop5.map((s) => ({ label: s.name, value: s.leave }))}
                  color="#16a34a"
                  max={maxLeaveForBar}
                />
                <p style={styles.smallNote}>* Based on Leave count only</p>
              </div>
              <div style={styles.sideCard}>
                ...
                <ul style={styles.featureList}>
                  ...
                  <li>✅ Top 5 Staff by Least Leave</li>
                </ul>
              </div>
            </div>
          </div>
...
const styles = {
  ...
  cardTitle: { fontSize: 13, fontWeight: 700, color: '#2563eb', marginTop: 0, marginBottom: 6, letterSpacing: 0.3 },
  sideSub: { fontSize: 12, color: '#6b7280', marginTop: 0, marginBottom: 14 },
  ...
  smallNote: { fontSize: 11, color: '#9ca3af', marginTop: 8, marginBottom: 0, textAlign: 'right' },
