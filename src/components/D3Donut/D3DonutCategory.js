...
//   ];
 const D3DonutCategory = ({ width, height,data,onClick }) => {
  const ref = useRef(null)
console.log('data',data);
  const radius = Math.min(width - 2 * MARGIN_X, height - 2 * MARGIN_Y) / 2
  const innerRadius = radius / 2
  const totalValues = data.reduce((acc, curr) => acc + curr.task_count, 0);
  const pie = useMemo(() => {
    const pieGenerator = d3.pie().value(d => d.task_count)
...
//   const arcGenerator = d3.arc()
  ...
  const shapes = pie.map((grp, i) => {
    ...
    const label = grp.data.first_name 
    // + " (" + grp.value + ")"
...
export default D3DonutCategory;